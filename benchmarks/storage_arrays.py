...
def generate_tensors(
    ...
    with timeblock("generate tensors | time: {time}"):
        ...
        for i in range(num_keys):
            tensors[str(i)] = torch.randn(num_examples, feature_size).numpy()
    return tensors


def benchmark_save_safetensors(tensors: dict[str, np.ndarray], path: Path) -> None:
...
def benchmark_load_safetensors(path: Path) -> dict[str, np.ndarray]:
...
def benchmark_save_pickle(tensors: dict[str, np.ndarray], path: Path) -> None:
...
def benchmark_load_pickle(path: Path) -> dict[str, np.ndarray]:
    logger.info(f"File size: {get_human_readable_file_size(path)}")
...
def main() -> None:
    ...
    n = 5
    for i in range(n):
        tensors = generate_tensors(num_examples=int(1e6), feature_size=128, num_keys=3)
