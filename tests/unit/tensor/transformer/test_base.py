...
def test_is_tensor_transformer_config_true() -> None:
    assert is_tensor_transformer_config({OBJECT_TARGET: "startorch.tensor.transformer.Identity"})
...
def test_setup_tensor_transformer_dict() -> None:
    assert isinstance(
        setup_tensor_transformer({OBJECT_TARGET: "startorch.tensor.transformer.Identity"}),
        Identity,
    )
